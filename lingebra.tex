...
\begin{definition}
Mějme vektorové prostory $\myspace{U}$, $\myspace{V}$, $\myspace{W}$ a bilineární zobrazení
z $\myspace{U}\times\myspace{V}$ do vektorového prostoru značeného jako $\myspace{U}\otimes\myspace{V}$
$\phi: \myspace{U}\times\myspace{V}\rightarrow\myspace{U}\otimes\myspace{V}$. Bilineární zobrazení
$\phi$ budeme nazývat \textit{tensorový součin} a prvky prostoru $\myspace{U}\otimes\myspace{V}$
\textit{tensory} právě tehdy když platí:
\begin{enumerate}	
\item množina obrazů $\left\lbrace\phi(u, v)|u\in\myspace{U}, v\in\myspace{V}\right\rbrace$ generuje celý
    prostor $\myspace{U}\otimes\myspace{V}$ (tzn. každý prvek prostoru lze vyjádřit jako lineární
    kombinaci prvků množiny obrazů).
\item Pro libovolné bilineární zobrazení $f: \myspace{U}\times\myspace{V}\rightarrow\myspace{W}$
    (tzn. $f\in\mathcal{B}(\myspace{U}\times\myspace{V}, \myspace{W})$) existuje lineární zobrazení
    $F: \myspace{U}\otimes\myspace{V}\rightarrow\myspace{W}$ takové, že $f(\myvec{u}, \myvec{v})
    = F(\phi(\myvec{u}, \myvec{v}))$ (nebo jinak $f = \phi\circ F$).
\end{enumerate}
\end{definition}

Na začátku kapitoly jsem sliboval, že tato definice bude nejvíce intuitivní a snadná. A jsem si dost
jistý, že vám teď tak nepřipadá. Proto si ji nyní rozebereme podrobněji. V této situaci v učebnicích
naleznete podobný obrázek:
\begin{center}
\includesvg[width=200pt]{universal_property}
\end{center}
Tentokrát výjimečně nebudu kritizovat, myslím si, že se jedná o hezkou visualizaci výše uvedeného.
Minimálně si po něm můžete během čtení jezdit prstem.

První bod definice je spíše technický a bude potřeba v následných důkazech. V tento okamžik
se jím příliš zabývat nemusíme.

Druhý bod je mnohem podstatnější. Umožňuje nám tzv. \textit{linearizovat bilineární
zobrazení}. Pomocí tensorového součinu dokážeme převést libovolné bilineární zobrazení na lineární
zobrazení. A to se nám vyplatí, protože s lineárním zobrazením už umíme pracovat.
...
skládat i bilineární formy. I to jsme v motivačním příkladu dělali - např. zobrazení integrující
polynom jsme složili dohromady s tensorem násobícím dva polynomy.

Samozřejmě, jak už to bývá, zde dochází ke zmatení pojmů. Ve fyzice se jako tensor označuje
právě ono pole hodnot, které representuje lineární zobrazení potažmo bilineární. My jsme ale
jako tensor označili prvky prostoru $\myspace{U}\otimes\myspace{V}$. Toto zaměňování vychází
z alternativní definice tensorů, kterou také časem (možná) probereme. Pro účely tohoto textu
budeme slovem \textit{tensor} označovat lineární zobrazení $F$, pokud explicitně neuvedeme,
že se jedná o prvek tensorového prostoru.
...
chová jako nějaká forma násobení lze vyjádřit tensorovou algebrou. Pokud jste dosud podobně
jako já přemýšleli nad obecným pravidlem popisujícím vyjadřovací sílu tensorů, nyní máte odpověď.
...
\begin{example}
Vezměme si vektorový prostor $\myspace{R}^2$ se standardním skalárním součinem. Pokud pracujeme
s ortonormální bází, skalární součin je daný předpisem
\begin{equation*}
\mycoord{x}\cdot\mycoord{y} = x_1 y_1 + x_2 y_2
\end{equation*}

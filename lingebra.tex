...
$\myvec{u}\otimes\myvec{v}$ - tím jsme vlastně řekli, že vracíme linearizaci.
...
Zobrazení je poměrně snadno injektivní (\textit{prosté}). Pokud bychom měli dvě různá lineární
zobrazení $L_1 \neq L_2$, pro která $\Gamma(L_1)=\Gamma(L_2)$, pak musí pro libovolné $\myvec{u}$
a libovolné $\varphi$ platit
\begin{equation*}
\varphi(L_1(\myvec{u})) = \varphi(L_2(\myvec{u}))
\end{equation*}
Protože lineární zobrazení nejsou stejná, musí
$\exists\myvec{u'}: L_1(\myvec{u'})\neq L_2(\myvec{u'})$. Pokud tento vektor dosadíme do předchozí
rovnice, získáme spor
\begin{equation*}
\varphi(L_1(\myvec{u'})) \neq \varphi(L_2(\myvec{u'}))
\end{equation*}
